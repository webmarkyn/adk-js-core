...
class DatabaseSessionService extends import_base_session_service.BaseSessionService {
  ...
  async createSession({ appName, userId, state, sessionId }) {
    ...
    const resolvedSessionId = cleanedSessionId || (0, import_env_aware_utils.randomUUID)();
    const now = Date.now();
  ...
  async listSessions({ appName, userId }) {
    ...
    try {
      ...
      const sessions = sessionResult.rows.map((row) => (0, import_session.createSession)({
        id: row.id,
        appName,
        userId,
        state: {},
  ...
  extractStateDelta(state) {
    ...
    for (const [key, value] of Object.entries(state)) {
      if (key.startsWith(import_state.State.APP_PREFIX)) {
        deltas.app[key.replace(import_state.State.APP_PREFIX, "")] = value;
      } else if (key.startsWith(import_state.State.USER_PREFIX)) {
        deltas.user[key.replace(import_state.State.USER_PREFIX, "")] = value;
  ...
  mergeState(appState, userState, sessionState) {
    ...
    for (const [key, value] of Object.entries(appState)) {
      mergedState[import_state.State.APP_PREFIX + key] = value;
    }
    for (const [key, value] of Object.entries(userState)) {
      mergedState[import_state.State.USER_PREFIX + key] = value;
    }
    return mergedState;
  }
  ...
  trimTempStateDelta(event) {
    ...
    const filteredEntries = Object.entries(event.actions.stateDelta).filter(([key]) => !key.startsWith(import_state.State.TEMP_PREFIX));

...
class DatabaseSessionService extends BaseSessionService {
  constructor(dbUrl, poolOptions = {}) {
    super();
    this.pool = new Pool(__spreadValues({
  ...
  async createSession({ appName, userId, state, sessionId }) {
    ...
    try {
      ...
      if (cleanedSessionId) {
        ...
        if (exists.rowCount) {
          throw new Error("Session with id ".concat(cleanedSessionId, " already exists."));
        }
      }
  ...
  mergeState(appState, userState, sessionState) {
    const mergedState = __spreadValues({}, sessionState);
  ...
  trimTempStateDelta(event) {
    ...
    return __spreadProps(__spreadValues({}, event), {
      actions: __spreadProps(__spreadValues({}, event.actions), {
